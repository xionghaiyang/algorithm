...
public class Code04_DoubleLinkedListQuickSort {
    ...
    private HeadTail process(Node L, Node R, int N) {
        if (L == null) {
            return null;
        }
        if (L == R) {
    ...
    private Info partition(Node L, Node pivot) {
        Node lh = null;
        Node lt = null;
        int ls = 0;
        Node rh = null;
    ...
    private String doubleLinkedListToString(Node head) {
        Node cur = head;
        Node end = null;
    ...
    public static void main(String[] args) {
        ...
        Code04_DoubleLinkedListQuickSort solution = new Code04_DoubleLinkedListQuickSort();
        for (int i = 0; i < testTime; i++) {
            int size = (int) (Math.random() * N);
            Node head1 = solution.generateRandomDoubleLinkedList(size, V);
            Node head2 = solution.cloneDoubleLinkedList(head1);
            Node sort1 = solution.quickSort(head1);
            Node sort2 = solution.sort(head2);
            if (!solution.equal(sort1, sort2)) {
