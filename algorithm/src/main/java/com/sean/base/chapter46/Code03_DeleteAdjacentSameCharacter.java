 ...
 * @Description: 如果一个字符相邻的位置没有相同字符，那么这个位置的字符出现不能被消掉
 * 比如:"ab"，其中a和b都不能被消掉
...
public class Code03_DeleteAdjacentSameCharacter {
        ...
        for (int L = 0; L < n; L++) {
            for (int R = L + 1; R < s.length(); R++) {
                if (canDelete(s.substring(L, R + 1))) {
                    minLen = Math.min(minLen, restMin1(s.substring(0, L) + s.substring(R + 1, n)));
                }
            }
        }
        return minLen;
    ...
    private boolean canDelete(String s) {
    ...
    //str[L..R]前面有没有跟着[L]字符，has T有 F无
    //L,R,has
    //最少能剩多少字符，消不了
    private int process(char[] str, int L, int R, boolean has) {
        if (L > R) {
            return 0;
        }
        if (L == R) {
    ...
    private int dpProcess(char[] str, int L, int R, boolean has, int[][][] dp) {
        if (L > R) {
            return 0;
        }
        int K = has ? 1 : 0;
    ...
    public String randomString(int len, int variety) {
    ...
    public static void main(String[] args) {
        Code03_DeleteAdjacentSameCharacter solution = new Code03_DeleteAdjacentSameCharacter();
        int maxLen = 16;
        int variety = 3;
        int testTime = 100000;
        ...
        for (int i = 0; i < testTime; i++) {
            ...
            String str = solution.randomString(len, variety);
            int res1 = solution.restMin1(str);
            int res2 = solution.restMin2(str);
            int res3 = solution.restMin3(str);
