...
public class Code05_HuffmanTree {
    ...
    //根据由文章生成词频表countMap,生成哈弗曼编码表
    //key:字符
    //value:该字符编码后的二进制形式
    //比如，频率表A:60,B:45,C:13,D:69,E:14,F:5,G:3
    //A 10
    //B 01
    //C 0011
    //D 11
    ...
    public String randomNumberString(int len, int range) {
    ...
    public static void main(String[] args) {
        Code05_HuffmanTree solution = new Code05_HuffmanTree();
        //根据词频表生成哈夫曼编码表
        ...
        map.put('F', 5);
        map.put('G', 3);
        HashMap<Character, String> huffmanForm = solution.huffmanForm(map);
        ...
        HashMap<Character, Integer> countMap = solution.countMap(str);
        //hf是根据countMap生成的哈夫曼编码表
        HashMap<Character, String> hf = solution.huffmanForm(countMap);
        //huffmanEncode是原始字符串转译后的哈夫曼编码
        String huffmanEncode = solution.huffmanEncode(str, hf);
        System.out.println(huffmanEncode);
        //huffmanDecode是哈夫曼编码还原成的原始字符串
        String huffmanDecode = solution.huffmanDecode(huffmanEncode, hf);
        ...
        for (int i = 0; i < testTime; i++) {
            ...
            String test = solution.randomNumberString(N, range);
            HashMap<Character, Integer> counts = solution.countMap(test);
            HashMap<Character, String> form = solution.huffmanForm(counts);
            String encode = solution.huffmanEncode(test, form);
            String decode = solution.huffmanDecode(encode, form);
