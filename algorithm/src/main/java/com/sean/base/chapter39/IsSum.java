...
public class IsSum {
    ...
    //暴力递归方法
    public boolean isSum1(int[] arr, int sum) {
        if (sum == 0) {
            return true;
        }
    ...
    private boolean process1(int[] arr, int i, int sum) {
        if (sum == 0) {
            return true;
        }
        if (i == -1) {
    ...
    public boolean isSum2(int[] arr, int sum) {
        if (sum == 0) {
            return true;
        }
        if (arr == null || arr.length == 0) {
    ...
    private boolean process2(int[] arr, int i, int sum, HashMap<Integer, HashMap<Integer, Boolean>> dp) {
    ...
    //arr中的值可能为正，可能为负，可能为0
    //自由选择arr中的数字，能不能累加到sum
    //经典动态规划
    public boolean isSum3(int[] arr, int sum) {
        if (sum == 0) {
            return true;
        }
        if (arr == null || arr.length == 0) {
    ...
    //此时如果arr的数字个数不算多(40以内)，那排其中的数值很大，分治的方法也将是最有解
    public boolean isSum4(int[] arr, int sum) {
        if (sum == 0) {
            return true;
        }
        if (arr == null || arr.length == 0) {
    ...
    private void process4(int[] arr, int i, int end, int pre, HashSet<Integer> res) {
        if (i == end) {
            res.add(pre);
        } else {
            process4(arr, i + 1, end, pre, res);
            process4(arr, i + 1, end, pre + arr[i], res);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testTime; i++) {
            ...
            int[] arr = solution.randomArray(size, M);
            int sum = (int) (Math.random() * ((M << 1) + 1)) - M;
            boolean res1 = solution.isSum1(arr, sum);
            boolean res2 = solution.isSum2(arr, sum);
            boolean res3 = solution.isSum3(arr, sum);
            boolean res4 = solution.isSum4(arr, sum);
            if (res1 ^ res2 || res1 ^ res3 || res1 ^ res4) {
                ...
                System.out.println("方法一答案:" + res1);
                System.out.println("方法二答案:" + res2);
                System.out.println("方法三答案:" + res3);
                System.out.println("方法四答案:" + res4);
                break;
            }
        }
