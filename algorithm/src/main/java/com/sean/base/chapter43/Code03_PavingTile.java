...
public class Code03_PavingTile {
    ...
    private int process(int[] pre, int level, int N) {
    ...
    private int dfs(int[] op, int col, int level, int N) {
        ...
        int ans = 0;
        //col位置不横摆
        ans += dfs(op, col + 1, level, N);//col位置上不摆横砖
        //col位置横摆，向右
    ...
    private int process2(int pre, int i, int N, int M) {
        if (i == N) {//base case
    ...
    private int dfs2(int op, int col, int level, int N, int M) {
        if (col == -1) {
    ...
    private int process3(int pre, int i, int N, int M, int[][] dp) {
        if (dp[pre][i] != -1) {
            return dp[pre][i];
        }
        int ans = 0;
    ...
    private int dfs3(int op, int col, int level, int N, int M, int[][] dp) {
        if (col == -1) {
    ...
    private void dfs4(int way, int op, int index, int end, int[] cur) {
        if (index == end) {
            cur[op] += way;
        } else {
            dfs4(way, op, index + 1, end, cur);
            if (((3 << index) & op) == 0) {//11<<index可以放砖
    ...
    public static void main(String[] args) {
        Code03_PavingTile solution = new Code03_PavingTile();
        int N = 8;
        int M = 6;
        System.out.println(solution.ways1(N, M));
        System.out.println(solution.ways2(N, M));
        System.out.println(solution.ways3(N, M));
        System.out.println(solution.ways4(N, M));
