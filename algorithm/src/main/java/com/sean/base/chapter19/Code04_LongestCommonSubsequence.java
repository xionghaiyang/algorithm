...
public class Code04_LongestCommonSubsequence {
    ...
    //可能的分类:
    //a)最长公共子序列，一定不以str[i]字符结尾，也一定不以str2[j]字符结尾
    //b)最长公共子序列,可能以str1[i]字符结尾，但是一定不以str2[j]字符结尾
    //c)最长公共子序列，一定不以str1[i]字符结尾，但是可能以str2[j]字符结尾
    //d)最长公共子序列，必须以str[i]字符结尾，也必须以str2[j]字符结尾
    //注意:a) b) c) d)并不是完全互斥的，它们可能会有重叠的情况
    //但是可以肯定，答案不会超过这四种可能性的范围
    //那么我们分别来看一下，这几种可能性怎么调用后续的递归
    //a)最长公共子序列，一定不以str1[i]字符结尾，也一定不以str2[j]字符结尾
    // 如果是这种情况，那么有没有str1[i]和str2[j]就根本不重要了。因为这两个字符一定没用啊
    // 所以就砍掉这两个字符，最长公共子序列 = str1[0..i-10]与str2[0..j-1]的最长公共子序列长度
    //b)最长公共子序列，可能以str1[i]字符结尾，但是一定不以str2[j]字符结尾
    // 如果是这种情况，那么我们可以确定str2[j]一定没有用，要砍掉；但是str1[i]可能有用，所以要保留
    // 所以，最长公共子序列=str1[0...i]和str2[0..j-1]的最长公共子序列长度(后续递归)
    //c) 最长公共子序列，一定不以str1[i]字符结尾，但是可能以str2[j]字符结尾
    // 根上面分析过程类似，最长公共子序列=str1[0..i-1]与str2[0..j]的最长公共子序列长度(后续递归)
    //d) 最长公共子序列，必须以str1[i]字符结尾也必须以str2[j]字符结尾
    // 同时可以看到，可能性d)存在的条件,一定是在str1[i]==str2[j]的情况才成立的
    // 随意，最长公共总长度=str1[0..i-1]与str2[0..j-1]的最长公共子序列长度(后续递归)
    ...
    //a中str1的范围<b)中str1的范围,a)中str2的范围==b)中str2的范围
    //所以a)不用求也知道,它比不过b)啊,因为右一个样本的范围比b)小啊
    //a)中str1的范围==c)中str1的范围,a)中str2的范围<c)中str2的范围
    //所以a)不用求也只到，它比不过c)啊,因为有一个样本的范围比c)小啊
    ...
    private int process(char[] str1, char[] str2, int i, int j) {
        if (i == 0 && j == 0) {
            //str1[0..0]和str2[0..0]都只剩下一个字符了
            //那如果字符相等，公共子序列长度就是1，不相等就是0
            //这显而易见
            return str1[i] == str2[j] ? 1 : 0;
        } else if (i == 0) {
            //这里的情况为
            //str1[0..0]和str2[0..j],str1只剩下一个字符了，但是str2不只右一个字符
            //因为str1只剩一个字符了，所以str1[0..0]和str2[0..j]的公共子序列最多长度为1
            //如果str1[0]==str2[j]，那么此时相等已经找到了！公共子序列长度就是1，也不可能更大了
            //如果str1[0]!=str2[j]，只是此时不相等而已
            //那么str2[0..j-1]上右没有字符等于str1[0]呢？不知道，所以递归继续找
            if (str1[i] == str2[j]) {
                return 1;
            } else {
            ...
            //因为str2只剩一个字符了，所以str[0..i]和str2[0..0]的公共子序列最多长度为1
            //如果str1[i]==str2[0]，那么此时相等已经找到了！公共子序列长度就是1，也不可能更大了
            //如果str1[i]!=str2[0]，只是此时不相等而已
            //那么str1[0..i-1]上右没有字符等于str2[0]呢？不知道，所以递归继续找
            if (str1[i] == str2[j]) {
                return 1;
            } else {
            ...
            //这里str1[0..i]和str2[0..j]，str1和str2都不只一个字符
            //看函数开始之前的注释部分
            //p1就是可能性c)
            ...
            //如果可能性d)不存在,即str1[i]!=str2[j]，那么就让p3等于0，然后去参与pk，反正不倾向
