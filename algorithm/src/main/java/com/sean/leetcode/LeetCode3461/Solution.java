package com.sean.leetcode.LeetCode3461;

/**
 * @Author xionghaiyang
 * @Date 2025-10-23 12:28
 * @Description https://leetcode.cn/problems/check-if-digits-are-equal-in-string-after-operations-i
 * 3461. 判断操作后字符串中的数字是否相等 I
 * 给你一个由数字组成的字符串 s 。
 * 重复执行以下操作，直到字符串恰好包含 两个 数字：
 * 从第一个数字开始，对于 s 中的每一对连续数字，计算这两个数字的和 模 10。
 * 用计算得到的新数字依次替换 s 的每一个字符，并保持原本的顺序。
 * 如果 s 最后剩下的两个数字 相同 ，返回 true 。
 * 否则，返回 false。
 * 3 <= s.length <= 100
 * s 仅由数字组成。
 */
public class Solution {
    ...
    private int comb(int n, int k) {
        return f[n] * invF[k] * invF[n - k] * pow(2, p2[n] - p2[k] - p2[n - k]) * pow(5, p5[n] - p5[k] - p5[n - k]) % MOD;
    }
