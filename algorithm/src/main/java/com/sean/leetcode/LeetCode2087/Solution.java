package com.sean.leetcode.LeetCode2087;

/**
 * @Author xionghaiyang
 * @Date 2026-04-04 06:53
 * @Description https://leetcode.cn/problems/minimum-cost-homecoming-of-a-robot-in-a-grid
 * 2087. 网格图中机器人回家的最小代价
...
public class Solution {
    ...
    public int minCost(int[] startPos, int[] homePos, int[] rowCosts, int[] colCosts) {
        ...
        int leftX = Math.min(startPos[0], homePos[0]), rightX = Math.max(startPos[0], homePos[0]);
        for (int x = leftX; x <= rightX; x++) {
            res += rowCosts[x];
        }
        int leftY = Math.min(startPos[1], homePos[1]), rightY = Math.max(startPos[1], homePos[1]);
