package com.sean.leetcode.LeetCode1958;

/**
 * @Auther: xionghaiyang
 * @Date: 2023-12-27 14:29
 * @Description: https://leetcode.cn/problems/check-if-move-is-legal/description/
 * 1958. 检查操作是否合法
    ...
    public boolean checkMove(char[][] board, int rMove, int cMove, char color) {
        ...
        for (int[] dir : dirs) {
            ...
            while (r >= 0 && r < 8 && c >= 0 && c < 8) {
                if (step >= 1 && board[r][c] == color) {
                    return true;
                }
                if (board[r][c] == '.') {
                    break;
                }
                if (step == 0 && board[r][c] == color) {
                    break;
                }
                step++;
                r += dr;
